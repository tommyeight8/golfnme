...
export class RoundService {
  async createRound(input: CreateRoundInput): Promise<RoundResult> {
    ...
    if (input.sessionId) {
      const session = await sessionRepository.findByIdWithDetails(input.sessionId)
  ...
  async getRound(id: string, userId: string): Promise<{ success: boolean; data?: RoundWithStats; error?: string }> {
  ...
  async deleteRound(id: string, userId: string): Promise<{ success: boolean; error?: string }> {
    const round = await roundRepository.findById(id)
  ...
  private calculateRoundStats(round: RoundWithDetails): RoundStats {
    ...
    const fairwayPercentage = eligibleFairways > 0
      ? ((round.fairwaysHit || 0) / eligibleFairways * 100).toFixed(1)
      : null

    const girPercentage = holesPlayed > 0
      ? ((round.greensInReg || 0) / holesPlayed * 100).toFixed(1)
      : null

    const avgPutts = holesPlayed > 0
