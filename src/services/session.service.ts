...
export class SessionService {
  ...
  async createSession(input: CreateSessionInput): Promise<SessionResult> {
    ...
    const activeSession = await sessionRepository.findActiveByUserId(input.hostId)
    if (activeSession) {
    ...
    const updatedSession = await sessionRepository.findByIdWithDetails(session.id)
   ...
   * Join a session
   */
  async joinSession(inviteCode: string, userId: string): Promise<SessionResult> {
    const session = await sessionRepository.findByInviteCode(inviteCode)
   ...
   * Leave a session
   */
  async leaveSession(sessionId: string, userId: string): Promise<{ success: boolean; error?: string }> {
    ...
    if (session.hostId === userId) {
      return { success: false, error: 'Host cannot leave. Cancel the session instead.' }
    }
   ...
   */
  async startSession(sessionId: string, hostId: string): Promise<SessionResult> {
    const session = await sessionRepository.findByIdWithDetails(sessionId)
    ...
    const updatedSession = await sessionRepository.findByIdWithDetails(sessionId)
   ...
   */
  async cancelSession(sessionId: string, hostId: string): Promise<{ success: boolean; error?: string }> {
