...
// import { formatScoreToPar, getScoreColor, getScoreRelativeToPar } from '@/lib/golf-utils'
// import type { GroupSession, LeaderboardEntry, Course, Hole } from '@/types'

// // Mock data
// const mockCourse: Course & { holes: Hole[] } = {
//   id: '1',
...
//     courseId: '1',
//     holeNumber: i + 1,
//     par: [1, 5, 9, 17].includes(i + 1) ? 5 : [3, 7, 12, 14].includes(i + 1) ? 3 : 4,
//     yardage: 350 + Math.floor(Math.random() * 200),
//     handicapRank: ((i * 3) % 18) + 1,
//   })),
// }
...
//     // In production, fetch session details and join
//     // joinSession(sessionData, user?.id || '')
...
//   const coursePar = mockCourse.holes.slice(0, myScores.filter(s => s > 0).length)
//     .reduce((a, h) => a + h.par, 0)
//   const myScoreToPar = myTotalScore - coursePar
...
//         const parThrough = mockCourse.holes.slice(0, currentHole).reduce((a, h) => a + h.par, 0)
//         return {
//           ...p,
//           scores: newScores,
//           totalScore: newTotalScore,
//           scoreToPar: newTotalScore - parThrough,
//           currentHole,
//           lastUpdate: new Date(),
//         }
//       }
...
//       setStrokes(mockCourse.holes[currentHole].par)
//     }
//   }
...
//     sendChatMessage(user?.id || '1', user?.name || 'You', chatInput)
//     setChatInput('')
//   }

//   // Reset strokes when hole changes
//   useEffect(() => {
//     setStrokes(myScores[currentHole - 1] || hole?.par || 4)
//   }, [currentHole])
...
//             <div className="text-center">
//               <p className="font-medium text-sm">{mockCourse.name}</p>
//               <div className="flex items-center justify-center gap-2 text-fairway-700/70 text-xs">
//                 <span className={`w-2 h-2 rounded-full ${isConnected ? 'bg-birdie' : 'bg-sand-400'}`} />
//                 <span>{players.length} players</span>
//               </div>
//             </div>
...
//               <div className="bg-sand-50 px-4 py-3 flex items-center justify-between border-b border-sand-100">
//                 <h3 className="font-medium text-sand-900">Players</h3>
//                 <span className="text-sm text-sand-500">{players.length} / 4</span>
//               </div>
//               <div className="divide-y divide-sand-100">
//                 {players.map((player, index) => (
//                   <div key={player.userId} className="flex items-center gap-4 p-4">
//                     <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
//                       index === 0 ? 'bg-gold-100 text-gold-600' : 'bg-fairway-100 text-fairway-600'
//                     }`}>
//                       {index === 0 ? <Crown className="w-5 h-5" /> : player.userName[0]}
//                     </div>
//                     <div className="flex-1">
//                       <p className="font-medium text-sand-900">{player.userName}</p>
//                       <p className="text-sm text-sand-500">
//                         {index === 0 ? 'Host' : 'Ready'}
//                       </p>
//                     </div>
...
//                 <div className="flex items-center gap-2">
//                   <Trophy className="w-5 h-5 text-fairway-700" />
//                   <h3 className="font-display font-semibold text-fairway-900">Leaderboard</h3>
//                 </div>
//                 <span className="text-sm text-fairway-700/70">Live</span>
//               </div>
//               <div className="divide-y divide-sand-100">
...
//                     layout
//                     className={`flex items-center gap-4 p-4 ${
//                       player.userName === 'You' ? 'bg-fairway-50' : ''
//                     }`}
//                   >
//                     <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
//                       index === 0 ? 'bg-gold-500 text-white' :
//                       index === 1 ? 'bg-sand-300 text-sand-700' :
...
//                         {player.userName}
//                         {player.userName === 'You' && (
//                           <span className="ml-2 text-xs text-fairway-600">(You)</span>
//                         )}
//                       </p>
//                       <p className="text-xs text-sand-500">
//                         Thru {player.scores.filter(s => s > 0).length}
...
//                         player.totalScore === 0 ? 'text-sand-300' :
//                         player.scoreToPar <= 0 ? 'text-birdie' : 'text-bogey'
//                       }`}>
//                         {player.totalScore > 0 ? formatScoreToPar(player.scoreToPar) : '—'}
//                       </p>
//                       <p className="text-sm text-sand-500">
//                         {player.totalScore || '—'}
//                       </p>
...
//                       getScoreColor(getScoreRelativeToPar(strokes, hole?.par || 4))
//                     }`}
//                   >
//                     {strokes}
//                   </motion.div>
...
//                 {/* Quick Buttons */}
//                 <div className="grid grid-cols-5 gap-2 mb-6">
//                   {[hole?.par - 2, hole?.par - 1, hole?.par, hole?.par + 1, hole?.par + 2].map(score => (
//                     score && score > 0 && (
//                       <button
//                         key={score}
//                         onClick={() => setStrokes(score)}
...
//             {/* Hole Progress */}
//             <div className="flex gap-1 overflow-x-auto pb-2">
//               {mockCourse.holes.map((h, i) => {
//                 const score = myScores[i]
//                 const relative = score ? getScoreRelativeToPar(score, h.par) : null
...
//             >
//               <div className="flex items-center justify-between p-4 border-b border-sand-100">
//                 <h3 className="font-display font-semibold text-sand-900">Group Chat</h3>
//                 <button onClick={() => setShowChat(false)}>
//                   <X className="w-5 h-5 text-sand-500" />
//                 </button>
//               </div>
