...
export default function StatsPage() {
  ...
  const [activeSection, setActiveSection] = useState<
    "overview" | "scoring" | "courses"
  >("overview");

  const scoringTotal = Object.values(mockStats.scoring).reduce(
    (a, b) => a + b,
    0
  );
  const scoringData = [
    {
      name: "Eagles",
      value: mockStats.scoring.eagles,
      color: "bg-eagle",
      pct: ((mockStats.scoring.eagles / scoringTotal) * 100).toFixed(1),
    },
    {
      name: "Birdies",
      value: mockStats.scoring.birdies,
      color: "bg-birdie",
      pct: ((mockStats.scoring.birdies / scoringTotal) * 100).toFixed(1),
    },
    {
      name: "Pars",
      value: mockStats.scoring.pars,
      color: "bg-par",
      pct: ((mockStats.scoring.pars / scoringTotal) * 100).toFixed(1),
    },
    {
      name: "Bogeys",
      value: mockStats.scoring.bogeys,
      color: "bg-bogey",
      pct: ((mockStats.scoring.bogeys / scoringTotal) * 100).toFixed(1),
    },
    {
      name: "Double+",
      value: mockStats.scoring.doubleBogeys + mockStats.scoring.triplePlus,
      color: "bg-double",
      pct: (
        ((mockStats.scoring.doubleBogeys + mockStats.scoring.triplePlus) /
          scoringTotal) *
        100
      ).toFixed(1),
    },
          ...
          <div className="card p-4">
            ...
            <p
              className={`text-xs font-medium mt-1 ${
                mockStats.overview.handicapTrend < 0
                  ? "text-birdie"
                  : "text-bogey"
              }`}
            >
              {mockStats.overview.handicapTrend > 0 ? "+" : ""}
              {mockStats.overview.handicapTrend} this month
            </p>
          </div>
        </section>
                ...
                {mockStats.recentRounds
                  ...
                  .map((round, i) => (
                    <span
                      ...
                      style={{
                        width: `${100 / mockStats.recentRounds.length}%`,
                      }}
                    >
                      {round.score}
                    </span>
              ...
              <div key={par} className="text-center">
                ...
                <p
                  className={`text-xs mt-1 font-medium ${
                    data.trend < 0 ? "text-birdie" : "text-bogey"
                  }`}
                >
                  {data.trend > 0 ? "+" : ""}
                  {data.trend.toFixed(2)} vs last month
                </p>
              </div>
            ))}
          </div>
