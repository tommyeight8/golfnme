...
export default function ActiveRoundPage() {
  ...
  const { round, scores, currentHole, setRound, setScore, setCurrentHole, clearRound } = useActiveRoundStore()
  ...
  const [courseData, setCourseData] = useState<(Course & { holes: Hole[] }) | null>(null)
    ...
    const loadRound = async () => {
      try {
        ...
        if (roundData.scores && roundData.scores.length > 0) {
          ...
          const playedHoles = new Set(roundData.scores.map((s: any) => s.hole.holeNumber))
          const nextHole = roundData.course.holes.find((h: Hole) => !playedHoles.has(h.holeNumber))
          if (nextHole) {
  ...
  const handleAbandonRound = async () => {
    if (confirm('Are you sure you want to abandon this round? Your progress will be saved.')) {
      try {
        ...
        {showMenu && (
          ...
          >
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-sand-50 transition-colors"
            >
              <BarChart3 className="w-5 h-5 text-sand-500" />
              <span className="text-sand-700">View Scorecard</span>
            </button>
            <button className="w-full flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-sand-50 transition-colors">
