...
import { getScoreRelativeToPar, getScoreColor, formatScoreToPar } from '@/lib/golf-utils'
...
export function Scorecard({
  ...
  const [strokes, setStrokes] = useState(existingScore?.strokes ?? hole?.par ?? 4)
  const [putts, setPutts] = useState(existingScore?.putts ?? 2)
  const [fairwayHit, setFairwayHit] = useState(existingScore?.fairwayHit ?? false)
  const [greenInReg, setGreenInReg] = useState(existingScore?.greenInReg ?? false)
  ...
  const scoreToPar = totalScore - totalPar + (existingScore ? 0 : strokes - (hole?.par ?? 0))
    ...
    <div className="card overflow-hidden">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="text-center">
          ...
          <p className={`mt-2 text-sm font-medium ${
            ...
            {strokes === hole.par ? 'Par' :
             strokes < hole.par ? `${hole.par - strokes} under` :
            ...
            >
              ...
              <div className="flex items-center justify-between p-4 bg-sand-50 rounded-xl">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="w-8 text-center font-bold text-lg">{putts}</span>
                  <button
              ...
              {hole.par >= 4 && (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-4 rounded-xl transition-colors ${
                    fairwayHit ? 'bg-birdie/10 border-2 border-birdie' : 'bg-sand-50'
                  }`}
                >
                  <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`w-full flex items-center justify-between p-4 rounded-xl transition-colors ${
                  greenInReg ? 'bg-birdie/10 border-2 border-birdie' : 'bg-sand-50'
                }`}
              >
                <div className="flex items-center gap-3">
      ...
      <div className="bg-sand-100 px-6 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xl font-bold text-sand-900">{totalScore + (existingScore ? 0 : strokes)}</p>
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <thead>
            <tr className="bg-fairway-500 text-white">
              ...
              {frontNine.map(h => (
                <th key={h.id} className="px-2 py-2 text-center font-medium w-10">
                  {h.holeNumber}
                </th>
              ))}
              <th className="px-2 py-2 text-center font-medium bg-fairway-600">OUT</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr>
              ...
              {frontNine.map(h => {
                ...
                const relativeScore = score ? getScoreRelativeToPar(score.strokes, h.par) : null
                const colorClass = relativeScore ? getScoreColor(relativeScore) : ''
                ...
                return (
                  ...
                  >
                    {score ? (
                      <span className={`inline-flex w-7 h-7 items-center justify-center rounded-full text-xs font-bold ${colorClass}`}>
                        {score.strokes}
                      </span>
                    ) : (
          ...
          <thead>
            <tr className="bg-fairway-500 text-white">
              ...
              <th className="px-2 py-2 text-center font-medium bg-fairway-600">IN</th>
            </tr>
          </thead>
          <tbody>
            <tr className="bg-sand-50">
